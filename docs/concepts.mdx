...
pyproject.toml
└── [tool.kensa]                # trace source + redaction model (kensa init)
...
You write evals and the `kensa_run` harness. Kensa generates ignored runtime artifacts under
`.kensa/`; `kensa doctor` derives readiness live without persisting it.
