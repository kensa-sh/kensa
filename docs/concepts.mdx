...
Kensa never guesses how to call your agent. Setup traces repository control flow to one
production-owned seam, cites it, and waits for approval before writing a thin
`tests/evals/conftest.py::kensa_run(case)` adapter. Otherwise it reports `cannot wire`.
...
`kensa doctor` rejects adapters that do not reach the approved boundary.
