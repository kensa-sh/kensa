...
`kensa.behavior_candidate.v1` contract. Its `semantic_fingerprint` hashes the normalized failure
pattern and expected outcome under the separately versioned `kensa.behavior_fingerprint.v1`
algorithm. Run-specific IDs, trace IDs, source, review status, current pass/fail state, proposed
checks, case shape, and operational risks do not affect identity. Equivalent behavior classes must
use the same canonical failure and outcome statements; the fingerprint does not infer equivalence
between differently worded prose.
