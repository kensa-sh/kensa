...
invokes its `respond(messages)` method and returns a `ConversationResult` containing canonical
messages, evaluated output, and one termination. `kensa doctor` checks that this harness reaches a
real agent boundary rather than a stub.
...
A **judge** is the semantic escape hatch: `judge(conversation, criteria, ...)` sends ordered
messages, normalized output, and termination as structured evidence. Use it only for criteria that
resist deterministic checks. See [Assertions](/assertions) and [Judge](/judge).
