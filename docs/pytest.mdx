...
`kensa eval` bounds the full setup, call, and teardown of each trial to 300 seconds by default. A marker `timeout_s` overrides `--trial-timeout`. On timeout, Kensa stops the suite immediately and preserves completed results, including the active operation published by `record_span`, `record_tool_call`, `record_llm_call`, or the built-in judge. Hard timeouts require macOS or Linux; explicit `timeout_s` values are rejected under direct `pytest` because there is no parent watchdog.
...
Judge calls have a separate 30-second advisory deadline. Built-in judge timeouts return an error-valued `JudgeResult` instead of ending the trial. Custom judge providers receive `timeout_s` and must honor it.

`kensa eval` rejects distributed pytest execution because the watchdog owns one active trial. Remove pytest-xdist worker options such as `-n` or `--numprocesses`, including options configured through pytest settings. Direct pytest remains unchanged.
