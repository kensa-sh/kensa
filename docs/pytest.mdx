...
`respond(messages)` once in direct mode and returns `ConversationResult`. Visible `content` becomes
...
async def test_sdr_draft(case, kensa_run, async_client):
    conversation = await case.run(kensa_run)
    ...
```
...
async def test_refund_dialogue(case, kensa_run):
    ...
    assert conversation.termination.reason in {"resolved", "max_turns"}
```
