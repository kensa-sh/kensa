...
`kensa eval` stops trials after 300 seconds by default and preserves available results; use `--trial-timeout` or the `timeout_s` marker option to change the limit.

Trial timeouts require macOS or Linux, work only with `kensa eval`, and leave detached subprocess cleanup to the eval.
