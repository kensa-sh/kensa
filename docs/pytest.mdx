...
`kensa eval` stops trials after 300 seconds by default and preserves results already received by the controller; use `--trial-timeout` or the `timeout_s` marker option to change the limit. Timeout artifacts set `complete` to `false`, include a structured `interruption`, and omit trials that did not report a result before termination.
