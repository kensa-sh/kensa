...
Inside an eval, the same helpers feed the live `kensa_trace` fixture. The case-aware agent returned
by `kensa_run(case)` wraps real tool and model calls with `record_tool_call` /
`record_llm_call`. Kensa adds one source-attributed outer response span for every attempted agent or
simulator response, while the existing trace totals include activity from both sources. See
...
`record_llm_call` also emits `gen_ai.operation.name`, `gen_ai.provider.name`, and
`gen_ai.request.model` when available. `operation_name` defaults to `chat`; pass `embeddings`,
`generate_content`, or `text_completion` for those OpenTelemetry GenAI operations. It uses the
`CLIENT` span kind by default; pass `span_kind=SpanKind.INTERNAL` for an in-process model. Kensa
reads cost and token counts from LLM spans. Anything that emits OTLP can be exported and imported
with `--from otlp`.
