...
During `kensa eval`, these context managers also publish the current operation to the parent watchdog. Ten-second heartbeats show the operation name plus allowlisted `attempt`, `turn`, `provider`, and `model` attributes. Heartbeat text is redacted and size-limited before it reaches terminal or CI logs. A trial timeout stores the full active operation in the local result artifact.
