...
To configure it from the environment instead, opt in with `KENSA_OTLP_EXPORT` and let the standard
OpenTelemetry variables supply the endpoint:

```bash
KENSA_OTLP_EXPORT=1 OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318 python my_agent.py
```

`KENSA_OTLP_EXPORT` accepts `1`, `true`, `yes`, or `on`. The standard `OTEL_EXPORTER_OTLP_*`
variables never enable export on their own, so `instrument()` stays a no-op in a process that only
configures OpenTelemetry for its own exporter. Opt in only where you want Kensa to export in
addition to whatever your application already sends, since both exporters ship the same spans.
