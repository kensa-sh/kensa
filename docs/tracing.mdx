...
You can use standard OpenTelemetry environment variables instead:

```bash
OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318 python my_agent.py
```

Set `OTEL_TRACES_EXPORTER` to a selection that excludes `otlp`, such as `none` or `console`, to
disable export enabled by either endpoint environment variable. An explicit `otlp_endpoint=`
argument takes precedence over that environment selection.
