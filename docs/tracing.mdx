...
The model choice is stored under `[tool.kensa]` in `pyproject.toml`. Kensa verifies the pinned
official wheel checksum on download. Later runs trust the Kensa-managed local cache after checking
the pinned model name, version, spaCy compatibility, and loadability; no readiness file is written.
...
During `kensa eval`, these context managers show the current operation in progress updates and preserve it when a trial times out.
