...
`gen_ai.request.model` when available. It uses the `CLIENT` span kind by default; pass
`span_kind=SpanKind.INTERNAL` for an in-process model. Kensa reads cost and token counts from LLM
spans. Anything that emits OTLP can be exported and imported with `--from otlp`.
