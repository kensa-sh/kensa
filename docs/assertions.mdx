...
def test_within_budget(case, kensa_run):
    ...
    assert result.trace.cost_usd < 0.10        # under 10 cents
...
|---|---|---|
| `result.trace.cost_usd` | `float` | Total trace cost in USD |
...
The `kensa_trace` fixture remains an independent accessor to the current trial trace. Use it when a
run can fail or time out before returning `CaseResult`, or to keep existing evals unchanged. Within a
managed trial, a successful run satisfies `result.trace is kensa_trace`.
