...
`case.run(...)` returns a compact `RunResult`: check visible dialogue in `.messages`, evaluated data
...
`case.run(kensa_run)` always returns `RunResult`. Assert on its typed fields with plain
Python.
