...
The `kensa_trace` fixture is your assertion helper for what the agent *did*. `case.run(...)` returns
a `ConversationResult`: check visible dialogue in `.messages`, evaluated data in `.output`, and the
stop condition in `.termination`.
...
`case.run(kensa_run)` always returns `ConversationResult`. Assert on its typed fields with plain
Python.
...
def test_output_shape(case, kensa_run):
    ...
    text = json.dumps(conversation.output)
    ...
    assert conversation.output["status"] == "resolved"

    # Canonical visible dialogue and termination are separate evidence.
    assert conversation.messages[-1]["role"] == "assistant"
    assert conversation.termination.reason == "direct"
