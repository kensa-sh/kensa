...
in `.output`, and the stop condition in `.termination`. The `kensa_trace` fixture separately exposes
what the agent *did*. Pass it to `judge(...)` only when the criterion needs trajectory evidence.
...
|---|---|---|
| `kensa_trace.cost_usd` | `float \| None` | Total cost, or `None` when any billable span is unpriced |
| `kensa_trace.known_cost_usd` | `float \| None` | Sum of priced spans, even when total cost is incomplete |
