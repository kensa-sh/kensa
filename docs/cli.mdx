...
Trials time out after 300 seconds by default, stopping the suite while preserving available results; use `--trial-timeout` or the `timeout_s` marker option to change the limit.

Trial timeouts require macOS or Linux, and evals remain responsible for cleaning up detached subprocesses.
