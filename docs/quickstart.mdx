...
Then ask your coding agent to run the generated `kensa-evals` skill. In interactive mode, `kensa init` asks for the first trace source: Langfuse, an existing trace export, or local Kensa trace capture. It saves the trace source and redaction model under `[tool.kensa]` in `pyproject.toml`. When trace evidence is configured, it also installs the mandatory redaction dependencies and prepares the small English model. Use `--redaction-model large` when you want the higher-recall model.
