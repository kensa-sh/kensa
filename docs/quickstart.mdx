...
The fixture owns case-aware construction and teardown. Kensa calls the returned instance's
`respond(messages)` method and returns one `ConversationResult` for both direct and simulated runs.
`kensa doctor` verifies the harness is wired to a real agent boundary before you trust any verdict.
