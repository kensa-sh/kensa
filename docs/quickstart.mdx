...
`kensa init` scaffolds the harness and installs `kensa-evals`. Setup follows repository source to a
production-owned seam and waits for approval before writing a thin `kensa_run` adapter. If no
faithful seam exists, it reports `cannot wire` instead of reconstructing the agent.

See [The harness fixture](/pytest#the-harness-fixture) for the adapter example and
[Skill guardrails](/skills#guardrails) for ownership and safety rules. `kensa doctor` verifies the
approved boundary before evidence collection begins.
