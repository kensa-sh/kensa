...
Then ask your coding agent to run the generated `kensa-evals` skill. In interactive mode, `kensa init` asks for the first trace source: Langfuse, an existing trace export, or local Kensa trace capture. When trace evidence is configured, it also installs the mandatory redaction dependencies and prepares the small English model. Use `--redaction-model large` when you want the higher-recall model.
