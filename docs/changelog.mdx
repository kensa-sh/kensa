...
- Only eval-required input, output, timing, status, tool, model, token, and cost fields are retained.
- Trace and span identifiers use import-local aliases, and Langfuse requests only required fields.
- `kensa init --redaction-model large` prepares the higher-recall redaction model.
