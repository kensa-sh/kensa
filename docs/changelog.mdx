...
Langfuse imports can now fetch complete traces in bounded, resumable batches.

- Each batch is limited by trace count, request count, and response bytes.
- Checkpoints resume legacy and observations-v2 imports without returning duplicate traces.
- Budget exhaustion returns completed progress when possible and fails without partial trace data otherwise.

[View the 0.26.0 release notes](https://github.com/kensa-sh/kensa/releases/tag/v0.26.0).

## 0.25.0

Trace fragments now have deterministic fingerprints for reliable identity checks.

- Fingerprints stay stable when span order or trace metadata changes.
- They change when the trace ID or span membership changes.

[View the 0.25.0 release notes](https://github.com/kensa-sh/kensa/releases/tag/v0.25.0).

## 0.24.0

Kensa can now process raw OTLP traces safely in memory.

- Converts OTLP protobuf payloads into redacted trace views without writing trace data to disk.
- Keeps trace and span IDs stable with a caller-provided key, with an option to remove usage and cost values.
- Rejects malformed or oversized payloads without returning partial results, and can be reused safely across concurrent calls.

[View the 0.24.0 release notes](https://github.com/kensa-sh/kensa/releases/tag/v0.24.0).

## 0.23.0

Judge calls now recover from transient LLM provider failures without exceeding the configured evaluation deadline.

- Synchronous and asynchronous completions retry rate limits, server errors, and recognized provider connection failures up to three total attempts with exponential backoff.
- Each retry receives only the remaining timeout budget, while invalid requests, Kensa timeouts, and other non-retryable errors fail immediately.
- Successful retried completions record their attempt count in result metadata for observability.

[View the 0.23.0 release notes](https://github.com/kensa-sh/kensa/releases/tag/v0.23.0).
...
> [!WARNING]
> Versions 0.1.0 through 0.8.0 are part of Kensa's legacy implementation and have been yanked from PyPI. Kensa 0.9.0 is a complete rewrite with a new API.
