...
Kensa aggregates the trials into a single verdict per case. `trials: 1` is reported as a smoke check; `trials > 1` is measured evidence of flakiness. See [Python SDK](/pytest) for verdict aggregation.
